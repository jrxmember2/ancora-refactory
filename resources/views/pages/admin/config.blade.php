...
<p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Nome, slogan, contatos e logos do sistema.</p>
            ...
            <form method="post" action="{{ route('config.branding.save') }}" enctype="multipart/form-data" class="mt-6 space-y-6">
                ...
                <div class="flex flex-wrap gap-3">
                    <button class="{{ $buttonClass }}">Salvar branding</button>
                </div>
            </form>
        </div>
        ...
        <div class="space-y-6">
            <div class="rounded-2xl border border-gray-200 bg-white p-6 shadow-theme-xs dark:border-gray-800 dark:bg-white/[0.03]" data-file-preview>
                ...
                <form method="post" action="{{ route('config.favicon.save') }}" enctype="multipart/form-data" class="mt-5 space-y-4">
                    ...
                    <button class="{{ $buttonClass }} w-full">Salvar favicon</button>
                </form>
            </div>
                    ...
                    <div class="grid grid-cols-1 gap-3 sm:grid-cols-2">
                        @foreach($modules as $module)
                            <label class="flex items-start gap-3 rounded-xl border border-gray-200 p-3 text-sm text-gray-700 dark:border-gray-800 dark:text-gray-200">
                                <input type="checkbox" name="enabled_modules[]" value="{{ $module->id }}" class="mt-1 rounded border-gray-300 text-brand-500 focus:ring-brand-500" @checked($module->is_enabled) @disabled(in_array($module->slug, ['dashboard', 'propostas', 'config'], true))>
                                <span>
    ...
    <div class="grid grid-cols-1 gap-6 xl:grid-cols-3" id="smtp-section">
        <div class="rounded-2xl border border-gray-200 bg-white p-6 shadow-theme-xs dark:border-gray-800 dark:bg-white/[0.03] xl:col-span-2">
            ...
            <div class="mt-6 space-y-5">
                @foreach($users as $user)
                    ...
                    <form method="post" action="{{ route('config.users.update', $user) }}" class="rounded-2xl border border-gray-200 p-5 dark:border-gray-800" x-data="{ markAllModules(){ $el.querySelectorAll('[data-modules] input[type=checkbox]:not(:disabled)').forEach(el => el.checked = true) }, markAllRoutes(){ $el.querySelectorAll('[data-routes] input[type=checkbox]:not(:disabled)').forEach(el => el.checked = true) } }">
                        @csrf
                        ...
                        <div class="mt-4 flex gap-2">
                            <button class="{{ $buttonClass }}">Salvar usuário</button>
                            @if(!$user->is_protected)
                                <button formaction="{{ route('config.users.delete', $user) }}" formmethod="POST" onclick="return confirm('Excluir usuário?')" class="rounded-xl border border-error-300 px-4 py-3 text-sm font-medium text-error-600">Excluir</button>
                            @endif
                        </div>
                    </form>
                @endforeach
        ...
        <div class="space-y-6">
            <div class="rounded-2xl border border-gray-200 bg-white p-6 shadow-theme-xs dark:border-gray-800 dark:bg-white/[0.03]" x-data="{
                profiles: {{ json_encode($accessProfiles) }},
                editProfile(slug) {
                    const p = this.profiles.find(x => x.slug === slug);
                    if (p) {
                        this.$refs.nameInput.value = p.name;
                        this.$refs.slugInput.value = p.slug;
                        this.$el.querySelectorAll('input[type=checkbox]').forEach(el => el.checked = false);
                        (p.module_ids || []).forEach(id => { const cb = this.$el.querySelector(`input[name='profile_modules[]'][value='${id}']`); if(cb) cb.checked = true; });
                        (p.route_ids || []).forEach(id => { const cb = this.$el.querySelector(`input[name='profile_routes[]'][value='${id}']`); if(cb) cb.checked = true; });
                ...
                <form method="post" action="{{ route('config.access-profiles.save') }}" class="mt-5 space-y-3">
                    @csrf
                    <input name="profile_name" x-ref="nameInput" placeholder="Nome do perfil" class="{{ $inputClass }}" required>
                    <input name="profile_slug" x-ref="slugInput" placeholder="Slug do perfil" class="{{ $inputClass }}" required>
                    <div>
                        <div class="mb-2 text-sm font-medium text-gray-700 dark:text-gray-300">Módulos do perfil</div>
                        <div class="grid grid-cols-1 gap-2 sm:grid-cols-2">
                            @foreach($modules as $module)
                                <label class="flex items-center gap-2 rounded-xl border border-gray-200 p-3 text-sm text-gray-700 dark:border-gray-800 dark:text-gray-300"><input type="checkbox" name="profile_modules[]" value="{{ $module->id }}"> {{ $module->name }}</label>
                            @endforeach
                        </div>
                    </div>
                    <div>
                        ...
                        <div class="max-h-60 space-y-2 overflow-auto rounded-2xl border border-gray-200 p-3 dark:border-gray-800">
                            @foreach($routeCatalog as $groupKey => $group)
                                <div>
                                    ...
                                    @foreach(($routePermissionGroups[$groupKey] ?? collect()) as $permission)
                                        <label class="mb-2 flex items-start gap-2 text-sm text-gray-700 dark:text-gray-300"><input type="checkbox" name="profile_routes[]" value="{{ $permission->id }}"> <span>{{ $permission->label }}</span></label>
                                    @endforeach
                                </div>
                            @endforeach
                        </div>
                    </div>
                    <button class="{{ $buttonClass }} w-full">Salvar perfil</button>
                </form>
                        ...
                        <form method="post" action="{{ route('config.access-profiles.delete', $profile['slug']) }}" class="rounded-2xl border border-gray-200 p-4 dark:border-gray-800">
                            ...
                            <div class="flex items-start justify-between gap-3">
                                ...
                                <div class="flex gap-2">
                                    <button type="button" @click="editProfile('{{ $profile['slug'] }}')" class="rounded-xl border border-brand-300 px-3 py-2 text-xs font-medium text-brand-600">Editar</button>
                                    <button class="rounded-xl border border-error-300 px-3 py-2 text-xs font-medium text-error-600">Excluir</button>
                                </div>
                            </div>
                        </form>
                    @empty
                        <div class="rounded-2xl border border-dashed border-gray-300 p-4 text-sm text-gray-500 dark:border-gray-700 dark:text-gray-400">Nenhum perfil de acesso cadastrado.</div>
                    @endforelse
                </div>
            </div>
            ...
            <div class="rounded-2xl border border-gray-200 bg-white p-6 shadow-theme-xs dark:border-gray-800 dark:bg-white/[0.03]">
                ...
                <form method="post" action="{{ route('config.smtp.save') }}" class="mt-5 space-y-3">
                    ...
                    <div class="flex flex-col gap-2 sm:flex-row">
                        <button type="submit" class="{{ $buttonClass }} flex-1">Salvar SMTP</button>
                        <button type="button" onclick="testSmtp(this)" class="rounded-xl border border-gray-300 px-4 py-3 text-sm font-medium text-gray-700 hover:bg-gray-50 dark:border-gray-700 dark:text-gray-200 dark:hover:bg-white/[0.03]">Testar Conexão</button>
                    </div>
                </form>
            </div>
        </div>
...
async function testSmtp(btn) {
    ...
    try {
        ...
        if(data.success) {
            showConfigToast(data.message, 'success');
        } else {
            showConfigToast(data.message || 'Erro na conexão.', 'error');
        }
    } catch (e) {
        showConfigToast('Erro na requisição. Verifique o console.', 'error');
    } finally {
        btn.disabled = false;
