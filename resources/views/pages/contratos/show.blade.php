...
<div class="grid grid-cols-1 gap-4 lg:grid-cols-4">
    <div class="rounded-2xl border border-gray-200 bg-white p-5 shadow-theme-xs dark:border-gray-800 dark:bg-white/[0.03]"><div class="text-xs uppercase tracking-[0.16em] text-gray-500 dark:text-gray-400">Cliente</div><div class="mt-2 text-lg font-semibold text-gray-900 dark:text-white">{{ $item->client?->display_name ?: 'Não informado' }}</div></div>
    <div class="rounded-2xl border border-gray-200 bg-white p-5 shadow-theme-xs dark:border-gray-800 dark:bg-white/[0.03]"><div class="text-xs uppercase tracking-[0.16em] text-gray-500 dark:text-gray-400">Condomínio</div><div class="mt-2 text-lg font-semibold text-gray-900 dark:text-white">{{ $item->condominium?->name ?: 'Não aplicável' }}</div></div>
...
<div class="mt-6 rounded-2xl border border-gray-200 bg-white p-2 shadow-theme-xs dark:border-gray-800 dark:bg-white/[0.03]">
    <div class="flex flex-wrap gap-2">
        @foreach(['resumo' => 'Resumo', 'historico' => 'Histórico', 'anexos' => 'Anexos'] as $tab => $label)
        ...
        <div class="space-y-6 xl:col-span-2">
            <div class="rounded-2xl border border-gray-200 bg-white p-6 shadow-theme-xs dark:border-gray-800 dark:bg-white/[0.03]">
                ...
                <dl class="mt-5 grid grid-cols-1 gap-4 md:grid-cols-2">
                    @foreach([
                        'Tipo' => $item->type,
                        'Categoria' => $item->category?->name ?: 'Não informada',
                        'Template' => $item->template?->name ?: 'Não informado',
                        'Responsável' => $item->responsible?->name ?: 'Não informado',
                        'Início' => optional($item->start_date)->format('d/m/Y') ?: 'Não informado',
                        'Término' => $item->indefinite_term ? 'Prazo indeterminado' : (optional($item->end_date)->format('d/m/Y') ?: 'Não informado'),
                        'Recorrência' => $item->recurrence ?: 'Não informada',
                        'Reajuste' => $item->adjustment_index ?: 'Não informado',
                    ] as $label => $value)
            ...
            <div class="rounded-2xl border border-gray-200 bg-white p-6 shadow-theme-xs dark:border-gray-800 dark:bg-white/[0.03]">
                ...
                <div class="prose prose-sm mt-5 max-w-none rounded-2xl border border-gray-100 p-5 dark:prose-invert dark:border-gray-800">{!! $item->content_html ?: '<p>Sem conteúdo salvo.</p>' !!}</div>
            </div>
        </div>
        ...
        <div class="space-y-6">
            <div class="rounded-2xl border border-gray-200 bg-white p-6 shadow-theme-xs dark:border-gray-800 dark:bg-white/[0.03]">
                ...
                <div class="mt-4 space-y-3 text-sm text-gray-700 dark:text-gray-200">
                    <div><span class="text-gray-500">Forma de cobrança:</span> {{ $item->billing_type ?: 'Não informada' }}</div>
                    <div><span class="text-gray-500">Dia de vencimento:</span> {{ $item->due_day ?: 'Não informado' }}</div>
                    <div><span class="text-gray-500">Gera cobrança automática:</span> {{ $item->generate_financial_entries ? 'Sim' : 'Não' }}</div>
                    <div><span class="text-gray-500">Centro de custo futuro:</span> {{ $item->cost_center_future ?: 'Não informado' }}</div>
                    <div><span class="text-gray-500">Categoria financeira futura:</span> {{ $item->financial_category_future ?: 'Não informada' }}</div>
                </div>
            </div>

            <div class="rounded-2xl border border-gray-200 bg-white p-6 shadow-theme-xs dark:border-gray-800 dark:bg-white/[0.03]">
                <h3 class="text-base font-semibold text-gray-900 dark:text-white">Ações rápidas</h3>
...
@elseif($activeTab === 'historico')
    <div class="mt-6 rounded-2xl border border-gray-200 bg-white p-6 shadow-theme-xs dark:border-gray-800 dark:bg-white/[0.03]">
        <h3 class="text-base font-semibold text-gray-900 dark:text-white">Histórico de versões</h3>
        <div class="mt-5 space-y-4">
            @forelse($item->versions as $version)
                <div class="rounded-2xl border border-gray-200 p-5 dark:border-gray-800">
                    <div class="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                        <div>
                            <div class="font-semibold text-gray-900 dark:text-white">Versão {{ $version->version_number }}</div>
                            <div class="mt-1 text-sm text-gray-500 dark:text-gray-400">{{ optional($version->generated_at)->format('d/m/Y H:i') ?: 'Data não informada' }} · {{ $version->generator?->name ?: 'Sistema' }}</div>
                            @if($version->notes)
            ...
            @empty
                <x-ancora.empty-state icon="fa-solid fa-clock-rotate-left" title="Sem versões geradas" subtitle="Gere o primeiro PDF do contrato para iniciar o histórico." />
            @endforelse
        </div>
    </div>
@else
    <div class="mt-6 grid grid-cols-1 gap-6 xl:grid-cols-3">
        <div class="rounded-2xl border border-gray-200 bg-white p-6 shadow-theme-xs dark:border-gray-800 dark:bg-white/[0.03]">
            ...
            <form method="post" action="{{ route('contratos.attachments.upload', $item) }}" enctype="multipart/form-data" class="mt-5 space-y-4">
                ...
                <select name="file_type" class="h-11 w-full rounded-xl border border-gray-300 bg-transparent px-4 text-sm text-gray-800 dark:border-gray-700 dark:text-white">
                    ...
                    <option value="ata_eleicao">ATA de eleição</option>
                    <option value="procuracao">Procuração</option>
                    <option value="comprovante">Comprovante</option>
                </select>
                <input name="description" class="h-11 w-full rounded-xl border border-gray-300 bg-transparent px-4 text-sm text-gray-800 dark:border-gray-700 dark:text-white" placeholder="Descrição do anexo">
        ...
        <div class="xl:col-span-2 rounded-2xl border border-gray-200 bg-white p-6 shadow-theme-xs dark:border-gray-800 dark:bg-white/[0.03]">
            ...
            <div class="mt-5 space-y-3">
                ...
                @empty
                    <x-ancora.empty-state icon="fa-solid fa-paperclip" title="Sem anexos" subtitle="Envie documentos vinculados a este contrato sem afetar o histórico principal." />
                @endforelse
            </div>
        </div>
    </div>
