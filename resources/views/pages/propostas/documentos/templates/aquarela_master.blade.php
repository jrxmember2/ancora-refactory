...
function premium_render_topbar(string $website): void
{
    ?>
    <div class="vb-topbar">
...
{
    $icons = [
        'email' => '<svg viewBox="0 0 24 24"><path d="M3 6.75A1.75 1.75 0 0 1 4.75 5h14.5A1.75 1.75 0 0 1 21 6.75v10.5A1.75 1.75 0 0 1 19.25 19H4.75A1.75 1.75 0 0 1 3 17.25V6.75Zm1.6.2 7.18 5.52a.4.4 0 0 0 .44 0l7.18-5.52a.35.35 0 0 0-.15-.06H4.75a.35.35 0 0 0-.15.06Zm14.8 1.67-6.27 4.82a1.9 1.9 0 0 1-2.32 0L4.6 8.62v8.63c0 .08.07.15.15.15h14.5c.08 0 .15-.07.15-.15V8.62Z"/></svg>',
        'instagram' => '<svg viewBox="0 0 24 24"><path d="M7.25 3h9.5A4.25 4.25 0 0 1 21 7.25v9.5A4.25 4.25 0 0 1 16.75 21h-9.5A4.25 4.25 0 0 1 3 16.75v-9.5A4.25 4.25 0 0 1 7.25 3Zm0 1.6A2.65 2.65 0 0 0 4.6 7.25v9.5a2.65 2.65 0 0 0 2.65 2.65h9.5a2.65 2.65 0 0 0 2.65-2.65v-9.5a2.65 2.65 0 0 0-2.65-2.65h-9.5Zm10.15 1.2a1.05 1.05 0 1 1 0 2.1 1.05 1.05 0 0 1 0-2.1ZM12 7.3A4.7 4.7 0 1 1 7.3 12 4.7 4.7 0 0 1 12 7.3Zm0 1.6A3.1 3.1 0 1 0 15.1 12 3.1 3.1 0 0 0 12 8.9Z"/></svg>',
        'phone' => '<svg viewBox="0 0 24 24"><path d="M6.62 2.9c.32-.08.66-.02.93.16l2.26 1.5c.43.28.62.82.47 1.31l-.68 2.16a1.2 1.2 0 0 0 .28 1.18l4.91 4.91a1.2 1.2 0 0 0 1.18.28l2.16-.68c.49-.15 1.03.04 1.31.47l1.5 2.26c.18.27.24.61.16.93-.34 1.39-1.56 2.39-3 2.39-8.3 0-15.03-6.73-15.03-15.03 0-1.44 1-2.66 2.39-3Zm.4 1.55c-.7.17-1.22.79-1.22 1.56 0 7.41 6.02 13.43 13.43 13.43.77 0 1.39-.52 1.56-1.22l-1.16-1.75-1.66.52a2.8 2.8 0 0 1-2.76-.66L10.3 11.4a2.8 2.8 0 0 1-.66-2.76l.52-1.66-1.75-1.16Z"/></svg>',
        'site' => '<svg viewBox="0 0 24 24"><path d="M4.75 4h14.5A1.75 1.75 0 0 1 21 5.75v12.5A1.75 1.75 0 0 1 19.25 20H4.75A1.75 1.75 0 0 1 3 18.25V5.75A1.75 1.75 0 0 1 4.75 4Zm0 1.6a.15.15 0 0 0-.15.15v1.5h14.8v-1.5a.15.15 0 0 0-.15-.15H4.75Zm-.15 3.25v9.4c0 .08.07.15.15.15h14.5c.08 0 .15-.07.15-.15v-9.4H4.6Zm2.3 1.5h4.9v1.4H6.9v-1.4Zm0 2.7h7.7v1.4H6.9v-1.4Z"/></svg>',
    ];
    return $icons[$type] ?? '';
}
...
function premium_scope_line_weight(string $line): int
{
    ...
    if ($length <= 130) return 2;
    if ($length <= 190) return 3;
    return 4;
}
...
function premium_paginate_scope(array $lines, int $budget, int $minTailWeight = 4): array
{
    ...
    $pages = []; $currentPage = []; $currentWeight = 0;
    foreach ($lines as $line) {
        $weight = premium_scope_line_weight((string) $line);
        if (!empty($currentPage) && ($currentWeight + $weight) > $budget) {
            $pages[] = $currentPage; $currentPage = []; $currentWeight = 0;
        }
        $currentPage[] = $line; $currentWeight += $weight;
    }
    if (!empty($currentPage)) $pages[] = $currentPage;
    if (count($pages) > 1) {
        $lastIndex = count($pages) - 1; $lastPage = $pages[$lastIndex]; $lastWeight = 0;
        foreach ($lastPage as $line) { $lastWeight += premium_scope_line_weight((string) $line); }
...
$investmentScopedCount = count(array_filter($preparedInvestmentOptions, static fn (array $option): bool => !empty($option['_has_scope'])));
...
<div class="proposal-page vb-cover-page">
    ...
    <?php premium_render_topbar((string) $branding['company_website']); ?>
    ...
    <div class="vb-cover-client-card">
        ...
        <?php if (!empty($document['attention_to'])): ?>
            <div class="vb-cover-client-card__line">
                ...
                <span>A/C: <?= htmlspecialchars($document['attention_to']); ?></span>
            </div>
        <?php endif; ?>
    </div>
...
<?php if ($hasScopePage = !empty($document['intro_context']) || !empty($scopeFirstPageLines)): ?>
    ...
    <div class="proposal-page vb-investment-page">
        ...
        <div class="vb-investment-grid">
            <?php foreach ($pageOptions as $option): ?>
                <article class="vb-investment-card <?= !empty($option['_is_featured']) ? 'is-featured' : ''; ?>">
                    <div class="vb-investment-card__main">
                        <?php if (!empty($option['_is_featured'])): ?>
                            <div class="vb-investment-badge">Opção Recomendada</div>
                        <?php endif; ?>
                    ...
                    <div class="vb-investment-card__side">
                        ...
                        <div class="vb-investment-payment-card">
                            <span class="vb-investment-payment-card__label">Pagamento</span>
        ...
        <div class="vb-contact-shell">
            <div class="vb-contact-card">
                <div class="vb-contact-item">
                    <span class="vb-contact-item__icon"><?= premium_contact_icon('phone'); ?></span>
                    <div>
                ...
                <div class="vb-contact-item">
                    <span class="vb-contact-item__icon"><?= premium_contact_icon('email'); ?></span>
                    <div>
                        <div class="vb-contact-item__label">E-mail</div>
            ...
            <div class="vb-contact-side">
                <div class="vb-contact-bubble">
                    <strong><?= htmlspecialchars($contactsCta['title'] ?? 'Pronto para transformar seu condomínio?'); ?></strong>
                    <span><?= htmlspecialchars($contactsCta['message'] ?? 'Vamos agendar uma conversa?'); ?></span>
                </div>
