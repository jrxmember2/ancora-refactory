...
@section('content')
<x-ancora.section-header :title="'Proposta '.$proposal->proposal_code" subtitle="Visualização consolidada da proposta no novo shell TailAdmin.">
    <div class="flex gap-3">
        ...
        <form method="post" action="{{ route('propostas.delete', $proposal) }}" onsubmit="return confirm('Deseja excluir esta proposta?');">
            @csrf
            <button class="inline-flex items-center gap-2 rounded-xl border border-error-300 px-4 py-3 text-sm font-medium text-error-600 dark:border-error-800 dark:text-error-400"><i class="fa-solid fa-trash"></i> Excluir</button>
        </form>
    </div>
...
<div class="grid grid-cols-1 gap-6 xl:grid-cols-[1.6fr,1fr]">
        ...
        <div class="rounded-2xl border border-gray-200 bg-white p-6 shadow-theme-xs dark:border-gray-800 dark:bg-white/[0.03]">
            ...
            @if($proposal->notes)
                <div class="mt-6 border-t border-gray-100 pt-6 dark:border-gray-800">
                    <p class="text-xs uppercase tracking-[0.2em] text-gray-400">Observações</p>
                    <p class="mt-3 whitespace-pre-line text-sm leading-7 text-gray-600 dark:text-gray-300">{{ $proposal->notes }}</p>
                </div>
            @endif
            @if($proposal->refusal_reason)
                <div class="mt-6 rounded-2xl border border-error-200 bg-error-50 px-5 py-4 text-sm text-error-700 dark:border-error-900/40 dark:bg-error-950/30 dark:text-error-300">
                    <strong>Motivo da recusa:</strong> {{ $proposal->refusal_reason }}
                </div>
            @endif
        </div>
    </div>
    ...
    <div class="space-y-6">
        <x-ancora.stat-card label="Anexos" :value="$proposal->attachments->count()" hint="Upload/download será plugado em seguida na nova base." icon="fa-solid fa-paperclip" />
        <div class="rounded-2xl border border-gray-200 bg-white p-6 shadow-theme-xs dark:border-gray-800 dark:bg-white/[0.03]">
            <h3 class="text-base font-semibold text-gray-900 dark:text-white">Histórico</h3>
            @if($proposal->history->count())
                <div class="mt-4 space-y-4">
                    @foreach($proposal->history->sortByDesc('created_at') as $event)
                        <div class="relative pl-5">
                            <div class="absolute top-1 left-0 h-2.5 w-2.5 rounded-full bg-brand-500"></div>
            ...
            @else
                <div class="mt-4"><x-ancora.empty-state icon="fa-solid fa-clock-rotate-left" title="Sem histórico" subtitle="O histórico desta proposta ainda está vazio." /></div>
            @endif
        </div>
    </div>
</div>
