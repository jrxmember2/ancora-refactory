...
<x-ancora.section-header title="Custas Processuais" subtitle="Mapeie custas, despesas judiciais, valores pagos e reembolso associado aos processos do escritorio.">
    <div class="flex flex-wrap gap-3">
        <a href="{{ route('financeiro.process-costs.create') }}" class="rounded-xl bg-brand-500 px-4 py-3 text-sm font-medium text-white">Nova custa</a>
        <x-financeiro.export-actions scope="process-costs" />
    </div>
...
<div class="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-theme-xs dark:border-gray-800 dark:bg-white/[0.03]">
    @if($items->count() === 0)
        <div class="p-6"><x-ancora.empty-state icon="fa-solid fa-gavel" title="Sem custas processuais" subtitle="Nenhuma custa foi registrada ate o momento." /></div>
    @else
                ...
                <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                    @foreach($items as $item)
                        <tr>
                            ...
                            <td class="px-4 py-3">{{ $item->cost_type ?: '-' }}</td>
                            <td class="px-4 py-3">{{ $item->category?->name ?: '-' }}</td>
                            <td class="px-4 py-3">{{ $money($item->amount) }}</td>
                            <td class="px-4 py-3">{{ $processCostStatuses[$item->status] ?? $item->status }}</td>
                            <td class="px-4 py-3 text-right">
                                <a href="{{ route('financeiro.process-costs.edit', $item) }}" class="rounded-xl border border-gray-200 px-3 py-2 text-xs font-medium dark:border-gray-700">Editar</a>
                            </td>
                        </tr>
                    @endforeach
                </tbody>
