                ...
                <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                    @foreach($items as $item)
                        ...
                        <tr>
                            ...
                            <td class="px-6 py-4">{{ $item->role_tag ?: '—' }}</td>
                            <td class="px-6 py-4">{{ strtoupper($item->entity_type) }}</td>
                            <td class="px-6 py-4">{{ $item->cpf_cnpj ?: '—' }}</td>
                            ...
                            <td class="px-6 py-4">
                                <div class="flex justify-end gap-2">
                                    <button type="button" onclick="document.getElementById('view-avulso-{{ $item->id }}').showModal()" class="rounded-lg border border-gray-200 px-3 py-2 text-xs font-medium">Visualizar</button>
                                    <a href="{{ route('clientes.avulsos.edit', $item) }}" class="rounded-lg border border-gray-200 px-3 py-2 text-xs font-medium">Editar</a>
                                    <form method="post" action="{{ route('clientes.avulsos.delete', $item) }}">@csrf @method('DELETE')<button onclick="return confirm('Excluir este cadastro?')" class="rounded-lg border border-error-300 px-3 py-2 text-xs font-medium text-error-600">Excluir</button></form>
                                </div>
