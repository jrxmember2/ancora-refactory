...
<div class="rounded-2xl border border-gray-200 bg-white p-6 shadow-theme-xs dark:border-gray-800 dark:bg-white/[0.03]">
    <form method="get" class="grid grid-cols-1 gap-4 md:grid-cols-2">
        <input type="search" name="q" value="{{ $filters['q'] ?? '' }}" placeholder="Buscar condomínio..." class="h-11 rounded-xl border border-gray-300 bg-transparent px-4">
        <div class="flex gap-3"><button class="rounded-xl bg-brand-500 px-4 py-3 text-sm font-medium text-white">Filtrar</button><a href="{{ route('clientes.condominios') }}" class="rounded-xl border border-gray-200 px-4 py-3 text-sm font-medium">Limpar</a></div>
    </form>
</div>
    ...
    @else
        <div class="overflow-x-auto">
            <table class="min-w-full text-left">
                <thead class="border-b border-gray-100 bg-gray-50 dark:border-gray-800 dark:bg-gray-900/40"><tr class="text-xs uppercase tracking-[0.16em] text-gray-500"><th class="px-6 py-4">Condomínio</th><th class="px-6 py-4">Tipo</th><th class="px-6 py-4">Síndico</th><th class="px-6 py-4">Blocos</th><th class="px-6 py-4 text-right">Ações</th></tr></thead>
                <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                    @foreach($items as $item)
                        ...
                        <tr>
                            ...
                            <td class="px-6 py-4">{{ $item->type?->name ?: '—' }}</td>
                            <td class="px-6 py-4">{{ $item->syndic?->display_name ?: '—' }}</td>
                            <td class="px-6 py-4">{{ $item->has_blocks ? 'Sim' : 'Não' }}</td>
                            <td class="px-6 py-4">
                                <div class="flex justify-end gap-2">
                                    <button type="button" onclick="document.getElementById('view-condo-{{ $item->id }}').showModal()" class="rounded-lg border border-gray-200 px-3 py-2 text-xs font-medium">Visualizar</button>
                                    <a href="{{ route('clientes.condominios.edit', $item) }}" class="rounded-lg border border-gray-200 px-3 py-2 text-xs font-medium">Editar</a>
                                    <form method="post" action="{{ route('clientes.condominios.delete', $item) }}">@csrf @method('DELETE')<button onclick="return confirm('Excluir este condomínio?')" class="rounded-lg border border-error-300 px-3 py-2 text-xs font-medium text-error-600">Excluir</button></form>
                                </div>
                                <dialog id="view-condo-{{ $item->id }}" class="w-full max-w-2xl rounded-3xl border border-gray-200 bg-white p-0 text-left shadow-2xl backdrop:bg-black/60 dark:border-gray-700 dark:bg-gray-900">
