...
class ClientsController extends Controller
{
    ...
    {
        return [
            'entity_type' => $request->input('entity_type', 'pf') === 'pj' ? 'pj' : 'pf',
            'profile_scope' => $scope,
            ...
            'opening_date' => trim((string) $request->input('opening_date', '')) ?: null,
            ...
            'inactive_reason' => trim((string) $request->input('inactive_reason', '')) ?: null,
            'contract_end_date' => trim((string) $request->input('contract_end_date', '')) ?: null,
    ...
    private function uploadAttachments(string $relatedType, int $relatedId, Request $request): void
    {
        ...
        $role = in_array($request->input('attachment_role', 'documento'), ['documento','contrato','outro'], true) ? $request->input('attachment_role') : 'documento';
        $dir = public_path('uploads/clientes/' . $relatedType . '/' . $relatedId);
        if (!is_dir($dir)) mkdir($dir, 0755, true);
        foreach ($files as $file) {
            if (!$file || !$file->isValid()) continue;
            $ext = strtolower($file->getClientOriginalExtension());
            if (!in_array($ext, ['pdf','png','jpg','jpeg','webp'], true)) continue;
            if ($role === 'contrato' && $ext !== 'pdf') continue;
    ...
    public function avulsos(Request $request): View
    {
        ...
        if ($term = trim((string) $request->input('q'))) {
            $query->where(fn ($sub) => $sub->where('display_name', 'like', "%{$term}%")->orWhere('legal_name', 'like', "%{$term}%")->orWhere('cpf_cnpj', 'like', "%{$term}%"));
        }
        if ($request->filled('entity_type')) $query->where('entity_type', $request->input('entity_type'));
        if ($request->filled('is_active')) $query->where('is_active', (int) $request->input('is_active'));
        return view('pages.clientes.avulsos', ['title' => 'Clientes avulsos', 'items' => $query->paginate(15)->withQueryString(), 'filters' => $request->all()]);
    }

    public function avulsoCreate(): View
    {
        return view('pages.clientes.avulsos-form', array_merge(['title' => 'Novo cliente avulso', 'item' => null, 'mode' => 'create', 'scope' => 'avulso', 'roleTag' => 'outro', 'attachments' => collect(), 'timeline' => collect()], $this->commonViewData()));
    }
    ...
    public function avulsoEdit(ClientEntity $avulso): View
    {
        ...
        return view('pages.clientes.avulsos-form', array_merge(['title' => 'Editar cliente avulso', 'item' => $avulso, 'mode' => 'edit', 'scope' => 'avulso', 'roleTag' => $avulso->role_tag, 'attachments' => ClientAttachment::query()->where('related_type','entity')->where('related_id',$avulso->id)->latest('id')->get(), 'timeline' => ClientTimeline::query()->where('related_type','entity')->where('related_id',$avulso->id)->latest('id')->get()], $this->commonViewData()));
    }
    ...
    public function contatos(Request $request): View
    {
        ...
        if ($request->filled('role_tag')) $query->where('role_tag', $request->input('role_tag'));
        return view('pages.clientes.contatos', ['title' => 'Contatos', 'items' => $query->paginate(15)->withQueryString(), 'filters' => $request->all()]);
    }

    public function contatoCreate(): View
    {
        return view('pages.clientes.contatos-form', array_merge(['title' => 'Novo contato', 'item' => null, 'mode' => 'create', 'attachments' => collect(), 'timeline' => collect()], $this->commonViewData()));
    }
    ...
    public function contatoStore(Request $request): RedirectResponse
    {
        ...
        return redirect()->route('clientes.contatos.edit', $entity)->with('success', 'Contato cadastrado.');
    }

    public function contatoEdit(ClientEntity $contato): View
    {
        abort_if($contato->profile_scope !== 'contato', 404);
        return view('pages.clientes.contatos-form', array_merge(['title' => 'Editar contato', 'item' => $contato, 'mode' => 'edit', 'attachments' => ClientAttachment::query()->where('related_type','entity')->where('related_id',$contato->id)->latest('id')->get(), 'timeline' => ClientTimeline::query()->where('related_type','entity')->where('related_id',$contato->id)->latest('id')->get()], $this->commonViewData()));
    }
    ...
    public function contatoDelete(ClientEntity $contato): RedirectResponse
    {
        abort_if($contato->profile_scope !== 'contato', 404);
        $contato->delete();
        return redirect()->route('clientes.contatos')->with('success', 'Contato excluído.');
    }
    ...
    public function condominios(Request $request): View
    {
        ...
        return view('pages.clientes.condominios', ['title' => 'Condomínios', 'items' => $query->paginate(15)->withQueryString(), 'filters' => $request->all()]);
    }

    public function condominioCreate(): View
    {
        return view('pages.clientes.condominios-form', array_merge(['title' => 'Novo condomínio', 'item' => null, 'mode' => 'create', 'attachments' => collect(), 'timeline' => collect(), 'blocksText' => ''], $this->commonViewData()));
    }
    ...
    {
        return view('pages.clientes.condominios-form', array_merge(['title' => 'Editar condomínio', 'item' => $condominio->load(['blocks','syndic','administradora','type']), 'mode' => 'edit', 'attachments' => ClientAttachment::query()->where('related_type','condominium')->where('related_id',$condominio->id)->latest('id')->get(), 'timeline' => ClientTimeline::query()->where('related_type','condominium')->where('related_id',$condominio->id)->latest('id')->get(), 'blocksText' => $condominio->blocks->pluck('name')->implode(PHP_EOL)], $this->commonViewData()));
    }
    ...
    public function unidades(Request $request): View
    {
        ...
        if ($request->filled('condominium_id')) $query->where('condominium_id', (int) $request->input('condominium_id'));
        return view('pages.clientes.unidades', ['title' => 'Unidades', 'items' => $query->paginate(15)->withQueryString(), 'filters' => $request->all()]);
    }

    public function unidadeCreate(): View
    {
        return view('pages.clientes.unidades-form', array_merge(['title' => 'Nova unidade', 'item' => null, 'mode' => 'create', 'attachments' => collect(), 'timeline' => collect()], $this->commonViewData()));
    }
    ...
    {
        return view('pages.clientes.unidades-form', array_merge(['title' => 'Editar unidade', 'item' => $unidade->load(['condominium','block','type','owner','tenant']), 'mode' => 'edit', 'attachments' => ClientAttachment::query()->where('related_type','unit')->where('related_id',$unidade->id)->latest('id')->get(), 'timeline' => ClientTimeline::query()->where('related_type','unit')->where('related_id',$unidade->id)->latest('id')->get()], $this->commonViewData()));
    }
    ...
    {
        return view('pages.clientes.config', ['title' => 'Configurações de clientes', 'types' => ClientType::query()->orderBy('scope')->orderBy('sort_order')->get()]);
    }
    ...
    {
        $errors = [];
        if (($payload['name'] ?? '') === '') $errors[] = 'Informe o nome do condomínio.';
        if (empty($payload['syndico_entity_id'])) $errors[] = 'Vincule um síndico ao condomínio.';
        if (empty($payload['is_active']) && empty($payload['inactive_reason'])) $errors[] = 'Informe o motivo da inativação.';
        return $errors;
    }
    ...
    private function unitPayload(Request $request): array
    {
        return [
            ...
            'owner_entity_id' => $request->integer('owner_entity_id') ?: null,
            'tenant_entity_id' => $request->integer('tenant_entity_id') ?: null,
    ...
    {
        $errors = [];
        if (empty($payload['condominium_id'])) $errors[] = 'Selecione o condomínio.';
        if (($payload['unit_number'] ?? '') === '') $errors[] = 'Informe o número da unidade.';
        return $errors;
    }
}
